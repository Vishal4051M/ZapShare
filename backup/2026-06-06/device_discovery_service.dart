...
class DeviceDiscoveryService {
    ...
    try {
      ...
      switch (messageType) {
        ...
        case 'ZAPSHARE_AUDIO_STOP':
          ...
          stopWebRtcAudio(datagram.address.address);
          break;
  ...
  Future<void> stopWebRtcAudio([String? targetIp]) async {
    if (targetIp != null) {
  ...
  void _attachAudioHandlers(_AudioSession session) {
    ...
    pc.onTrack = (event) async {
      ...
      debugPrint('🔊 WebRTC onTrack: ${track.kind} (id: ${track.id}, label: ${track.label})');
      
      if (track.kind != 'audio') return;
      if (event.streams.isEmpty) {
        debugPrint('⚠️ [WEB-RTC] Audio track received but no streams attached');
        return;
      }
      ...
      try {
        debugPrint('🎧 Attaching remote audio stream to renderer');
        _audioRenderer?.srcObject = session.remoteStream;
        await _audioRenderer?.setVolume(1.0);
        
        // CRITICAL: Start foreground service on receiver end too
        await _startAudioForeground();
        _audioActiveController.add(true);
        ...
        debugPrint('❌ [WEB-RTC] Error attaching remote audio: $e');
      }
    };
    ...
    pc.onConnectionState = (state) async {
      if (state == RTCPeerConnectionState.RTCPeerConnectionStateFailed ||
        ...
        await stopWebRtcAudio(session.peerIp);
      }
    };
  }

  Future<void> _sendAudioOfferPayload(String targetIp, _AudioSession session) async {
    print('📡 [Discovery] Preparing AUDIO_OFFER payload for $targetIp');
  ...
  String _mungeAudioSdp(String sdp) {
    ...
    // Reverting to 10ms frames for ultra-low latency as requested.
    if (newSdp.contains('useinbandfec=1')) {
      newSdp = newSdp.replaceAll(
        'useinbandfec=1', 
        'useinbandfec=1; ptime=10; minptime=10; maxaveragebitrate=128000; stereo=1'
      );
    } else {
      final regExp = RegExp(r'a=fmtp:(\d+) (.+)');
      newSdp = newSdp.replaceAllMapped(regExp, (match) {
        if (match.group(2)!.contains('opus')) {
          return '${match.group(0)}; ptime=10; minptime=10; maxaveragebitrate=128000; stereo=1';
        }
