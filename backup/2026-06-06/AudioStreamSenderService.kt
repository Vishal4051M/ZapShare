...
class AudioStreamSenderService : Service() {
    ...
    private fun startStreaming(resultCode: Int, resultData: Intent) {
        ...
        // ==================== Thread 3: Send ====================
        // ====== IMPROVEMENT: Paced sending ======
        // Instead of sending as fast as the queue fills, pace sends to ~1 packet
        // per 10ms. This prevents burst-sending after a queue buildup, which would
        // overwhelm the receiver's jitter buffer and router queues.
        sendThread = Thread {
            android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO)
            Log.d("ZapShare", "Send thread started (paced)")
            val frameDurationNs = 20 * 1_000_000L  // 20ms
            ...
            while (isRunning) {
                ...
                // Don't send config packets too fast in bursts
                val isConfig = packetData.isNotEmpty() && packetData[0].toInt() == 0x02
                ...
                // ====== Pacing: don't send faster than real-time ======
                // This prevents burst-sending after the encode thread catches up
                // from a backlog. Without pacing, 10 frames arrive at the receiver
                // in <1ms, overwhelming its jitter buffer.
                if (!isConfig) {
                    nextSendTimeNs += frameDurationNs
                    val now = System.nanoTime()
                    val sleepNs = nextSendTimeNs - now
                    if (sleepNs > 1_000_000L) {
                        try {
                            Thread.sleep(sleepNs / 1_000_000L, (sleepNs % 1_000_000L).toInt())
                        } catch (e: Exception) {}
                    } else if (sleepNs < -frameDurationNs) {
                        // Reset immediately if even 1 frame behind (prevents latency buildup):
                        nextSendTimeNs = now + frameDurationNs
                    }
                }
