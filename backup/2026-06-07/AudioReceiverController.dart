...
class AudioReceiverController extends ChangeNotifier {
  ...
  AudioReceiverController({required this.senderIp, this.useLanAudio = true, this.audioUrl});
  ...
  Future<void> _startReceiving() async {
    ...
    try {
      ...
      if (Platform.isLinux || Platform.isWindows) {
        // For Windows/Linux we use the Dart/MPV path
  ...
  void dispose() {
    ...
    if (Platform.isLinux || Platform.isWindows) {
      ...
      // Only stop WebRTC if it was actually used
      // LAN audio sessions don't need WebRTC teardown
      if (!useLanAudio) {
