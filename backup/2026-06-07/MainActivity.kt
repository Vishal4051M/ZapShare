...
class ScreenMirrorService : android.app.Service() {
    ...
    private class MjpegServer(
        ...
        fun stopServer() {
            ...
            // Signal all pushing threads to stop
            frameLock.lock()
            try { frameCondition.signalAll() } finally { frameLock.unlock() }
        ...
        override fun run() {
                ...
                while (isRunning) {
                    ...
                    if (frame != null && clients.isNotEmpty()) {
                        ...
                        synchronized(clients) {
                            for (client in clients) {
                                try {
                                    // Use a smaller write check if possible, but for MJPEG we just push
                                    client.write(headerBytes)
                                    client.write(frame)
                                    client.write(endBytes)
                                    // Important: Don't flush inside a loop if we have many clients, 
                                    // but for 1-2 it's okay. We keep it for real-time delivery.
                                    client.flush()
                                    pushedFrames++
                                } catch (_: Exception) {
                                    dead.add(client)
            ...
            }.also { it.isDaemon = true; it.name = "MjpegPush"; it.priority = Thread.MAX_PRIORITY }.start()
                ...
                try {
                    ...
                    Thread {
                        try {
                            ...
                            // Don't use BufferedOutputStream for audio, it introduces massive delay
                            // We want immediate packet dispatch down the socket
                            val rawOutput = client.getOutputStream()
                            val isAudioRequest = requestPath == "/audio" && audioRecord != null

                            if (isAudioRequest) {
                                // Shrink TCP network window to 4KB (a fraction of a second)
                                // This causes the OS to drop stale audio packets instead of queueing them in the network
