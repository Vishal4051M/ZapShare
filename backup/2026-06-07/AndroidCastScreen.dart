...
class _AndroidCastScreenState extends State<AndroidCastScreen>
  ...
  // Audio cast state
  String? _selectedAudioUri;
  String? _selectedAudioName;
  String? _audioServerUrl;
  bool _isAudioServerRunning = false;
  final Set<String> _audioTargets = {};
  bool _audioCastUsingMirror = false;
  bool _preferLowLatencyAudio = false;
  ...
  Future<void> _refreshAccessibilityStatus() async {
    try {
      ...
      // If platform call fails, assume enabled to avoid blocking the user.
      if (mounted) setState(() => _isAccessibilityEnabled = true);
    }
  }
  ...
  Future<void> _requestAccessibility() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Unable to open accessibility settings: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
    ...
    try {
      ...
      if (result != null && result.isNotEmpty) {
        ...
        setState(() {
          ...
          _selectedAudioName = docFile.name;
        });
        await _startAudioServer();
      }
    } catch (e) {
  ...
  Future<void> _stopAudioServer() async {
    if (_isAudioServerRunning) {
      try {
        if (_audioCastUsingMirror) {
        ...
        print('Error stopping audio server: $e');
      }
      setState(() {
        _isAudioServerRunning = false;
        _audioServerUrl = null;
        _audioCastUsingMirror = false;
      });
    }
  }

  Future<bool> _startWebRtcAudioCast(String targetIp) async {
    try {
      final ok = await _discoveryService.startWebRtcAudio(targetIp);
      return ok;
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('WebRTC audio failed: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<bool> _startSystemAudioCast() async {
    ...
    try {
      // Request screen capture permission (required for AudioPlaybackCapture)
      bool granted = false;
      try {
      ...
      if (port <= 0) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Audio capture failed: $lastError'),
              backgroundColor: Colors.red,
            ),
          );
        }
      ...
      if (ip == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Could not determine local IP for audio cast'),
              backgroundColor: Colors.red,
            ),
          );
        }
      ...
      _audioServerUrl = 'http://$ip:$port/audio';
      _isAudioServerRunning = true;
      _audioCastUsingMirror = true;
      if (mounted) setState(() {});
      return true;
    } catch (e) {
      print('System audio start error: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Audio capture error: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _startServer() async {
    ...
    try {
      ...
      if (ip == null)
        throw Exception(
          'Could not determine local IP address. Please ensure you are connected to WiFi.',
        );
      ...
      if (port == null || port <= 0)
        throw Exception('Server started but port is invalid. $lastError');
  ...
  Future<void> _startAudioServer() async {
    if (_selectedAudioUri == null) return;
    ...
    try {
      ...
      try {
        await _channel.invokeMethod('startForegroundService', {
          'title': 'Casting Audio',
          'content': 'ZapShare is streaming ${_selectedAudioName ?? "audio"}',
        });
      } catch (e) {
        print('⚠️ Foreground service (audio) warning: $e');
      }
      ...
      for (int attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          lastError = e.toString();
          print('⚠️ Audio server attempt ${attempt + 1} failed: $e');
          if (attempt < 2) {
            try {
              await _channel.invokeMethod('stopVideoServer');
            } catch (_) {}
      ...
      if (mounted) setState(() {});
      print('🎵 Audio server started at $_audioServerUrl');
    } catch (e) {
      print('❌ Error starting audio server: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Audio server error: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _broadcastAudio() async {
    var useLegacyPath = !_preferLowLatencyAudio;
    String? targetIp;

    if (_preferLowLatencyAudio) {
      if (_audioTargets.length != 1) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Select exactly one device for WebRTC audio'),
            backgroundColor: Colors.orange,
          ),
        );
        return;
      }
      targetIp = _audioTargets.first;
      final ok = await _startWebRtcAudioCast(targetIp);
      if (!ok) {
        useLegacyPath = true; // fallback to HTTP/MJPEG
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('WebRTC audio started with $targetIp'),
              backgroundColor: const Color(0xFFFFD600),
            ),
          );
        }
    ...
    if (_audioTargets.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Select at least one device'),
          backgroundColor: Colors.orange,
        ),
      );
      return;
    ...
    if (!useLegacyPath && targetIp != null) {
      // WebRTC path already negotiated above
    }

    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Broadcasting to ${_audioTargets.length} device(s)'),
        backgroundColor: const Color(0xFFFFD600),
        behavior: SnackBarBehavior.floating,
      ),
    );
  ...
  void _toggleAudioTarget(String ip) {
    setState(() {
      if (_audioTargets.contains(ip)) {
        _audioTargets.remove(ip);
      } else {
        _audioTargets.add(ip);
      }
    });
  }
  ...
  Future<void> _startScreenMirror(DiscoveredDevice device) async {
    print('\n🪞 ═══════════════════════════════════════════════════');
    print(
      '🪞 [ScreenMirror] START - Target: ${device.deviceName} (${device.ipAddress})',
    );
    ...
    try {
      ...
      print(
        '🪞 [ScreenMirror] Step 1: Stopping any existing mirror session...',
      );
      try {
        await _channel.invokeMethod('stopScreenMirror');
        await Future.delayed(const Duration(milliseconds: 500));
      ...
      print(
        '🪞 [ScreenMirror] Step 2: Requesting microphone permission for audio capture...',
      );
      try {
        ...
        if (!micStatus.isGranted) {
          print(
            '🪞 [ScreenMirror] Step 2: Microphone denied - audio will not be shared',
          );
        }
      } catch (e) {
        print(
          '🪞 [ScreenMirror] Step 2: Microphone permission request failed: $e',
        );
      }

      // Step 3: Request screen capture permission
      print(
        '🪞 [ScreenMirror] Step 3: Requesting screen capture permission...',
      );
      bool granted = false;
      try {
        granted = await _channel.invokeMethod('requestScreenCapture');
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Screen capture request failed: ${e.toString().contains('SecurityException') ? 'Permission denied by system' : e}',
              ),
              backgroundColor: Colors.red,
              behavior: SnackBarBehavior.floating,
            ),
      ...
      print(
        '🪞 [ScreenMirror] Step 3: Polling for screen mirror port (up to 25 attempts)...',
      );
      int port = 0;
      ...
      for (int attempt = 0; attempt < 25; attempt++) {
        ...
        try {
          final result = await _channel.invokeMethod('getScreenMirrorPort');
          print(
            '🪞 [ScreenMirror] Step 3: Attempt $attempt - getScreenMirrorPort returned: $result (type: ${result.runtimeType})',
          );
          if (result is int && result > 0) {
            port = result;
            print(
              '🪞 [ScreenMirror] Step 3: Got valid port = $port on attempt $attempt',
            );
            break;
          }
          ...
          if (result is int && result == -1) {
            ...
            print(
              '🪞 [ScreenMirror] Step 3: Native returned -1 error on attempt $attempt',
            );
          }
        } catch (e) {
          lastError = e.toString();
          print(
            '⚠️ [ScreenMirror] Step 3: getScreenMirrorPort attempt $attempt FAILED: $e',
          );
        }
      }
      if (port <= 0) {
        ...
        print(
          '⚠️ [ScreenMirror] Step 3: All 25 attempts failed. Last error: $lastError',
        );
        print(
          '⚠️ [ScreenMirror] Step 3: Retrying - will restart screen mirror service...',
        );
        try {
          await _channel.invokeMethod('stopScreenMirror');
          await Future.delayed(const Duration(milliseconds: 1000));
          print(
            '⚠️ [ScreenMirror] Step 3: Retry - requesting screen capture again...',
          );
          ...
          print(
            '⚠️ [ScreenMirror] Step 3: Retry - getScreenMirrorPort returned: $retryPort',
          );
          if (retryPort is int && retryPort > 0) {
            ...
            print(
              '⚠️ [ScreenMirror] Step 3: Retry FAILED - invalid port: $retryPort',
            );
          }
        } catch (e) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Column(
                ...
                children: [
                  const Text(
                    'Failed to start screen mirror server',
                    style: TextStyle(fontWeight: FontWeight.bold),
                  ),
                  const SizedBox(height: 4),
                  Text(
                  ...
                  const SizedBox(height: 4),
                  const Text(
                    'Try: Restart the app or check if another app is using screen capture',
      ...
      if (ip == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Could not determine local IP address. Make sure you are connected to WiFi.',
              ),
              backgroundColor: Colors.red,
              behavior: SnackBarBehavior.floating,
      ...
      print(
        '🪞 [ScreenMirror] Step 5: Verifying server reachability at $ip:$port...',
      );
      ...
      if (!serverReachable) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text(
                'Screen mirror server started but is not reachable. Check firewall settings.',
              ),
              backgroundColor: Colors.orange,
              behavior: SnackBarBehavior.floating,
      ...
      print(
        '🪞 [ScreenMirror] Step 6: Sending screen mirror request to ${device.ipAddress}...',
      );
      print(
        '🪞 [ScreenMirror] Step 6: streamUrl=$streamUrl, targetIp=${device.ipAddress}',
      );
      ...
      print(
        '❌ Platform error starting screen mirror: ${e.code} - ${e.message}',
      );
      if (mounted) {
        ...
        if (e.code == 'PERMISSION_DENIED') {
          ...
          userMessage =
              'Screen capture service failed to start. Try restarting the app.';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Screen mirror error: ${e.toString().length > 100 ? '${e.toString().substring(0, 100)}...' : e}',
            ),
            backgroundColor: Colors.red,
            behavior: SnackBarBehavior.floating,
            duration: const Duration(seconds: 4),
          ...
          body: SafeArea(
            child: CustomScrollView(
              slivers: [
                ...
                // Accessibility Banner (Screen Mirror mode only)
                if (_mode == CastMode.screenMirror && !_isAccessibilityEnabled)
                  SliverToBoxAdapter(
                    child: Padding(
                      padding: const EdgeInsets.symmetric(
                        horizontal: 24,
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'To allow the computer to control this device, please enable ZapShare in Accessibility Settings.',
                              style: GoogleFonts.outfit(
                                fontSize: 13,
...
class _RippleEffect extends StatefulWidget {
  ...
  const _RippleEffect({this.size = 300, this.color = Colors.black});
