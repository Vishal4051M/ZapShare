...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        nativeCastChannel!!.setMethodCallHandler { call, result ->
            when (call.method) {
                "castCommand" -> {
                    ...
                    val handled = NativeVideoPlayerActivity.handleCastCommandFromDart(action, seekPos, volume, trackIndex)
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(
            ...
            android.content.IntentFilter("com.example.zap_share.AUDIO_STATE")
        )
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(
            object : BroadcastReceiver() {
                override fun onReceive(context: Context, intent: Intent) {
                    ...
                    runOnUiThread {
                        methodChannel?.invokeMethod("remoteCommand", mapOf("action" to action))
                    }
                }
            },
            android.content.IntentFilter("com.example.zap_share.CAST_COMMAND")
        )
            ...
            when (call.method) {
               ...
               "getFileSize" -> {
                ...
                "startForegroundService" -> {
                    ...
                    startForegroundServiceCompat(title, content, useMediaProjection)
                    result.success(true)
                }
    ...
    private fun handleShareIntent(intent: Intent?) {
        ...
        if (filesMap.isNotEmpty()) {
            val filesJson = JSONArray(filesMap).toString()
            
            // Cold start - store for retrieval via getInitialSharedFiles
            initialSharedFiles = filesJson
            
            // Warm start - send immediately if possible
            if (methodChannel != null) {
                println("🚀 [MainActivity] Sending ${filesMap.size} shared files via MethodChannel")
                methodChannel?.invokeMethod("sharedFiles", filesMap)
            } else {
                println("🚀 [MainActivity] Caching ${filesMap.size} shared files for cold start")
            }
        }
    }
    ...
    private class VideoWebServer(
        ...
        private fun serveFile(output: java.io.OutputStream, uri: Uri, start: Long, end: Long, method: String, id: String) {
            ...
            try {
                ...
                if (afd == null) {
                    android.util.Log.e("ZapShareServer", "Could not open AssetFileDescriptor for $uri")
                    send404(output)
                    return
                }
                ...
                if (method != "HEAD" && method != "OPTIONS") {
                   ...
                   // PRO-TIP: For truly large files (18GB-20GB), zero-copy transfer is 
                   // significantly more memory-efficient and resilient to GC pauses.
    ...
    private fun startForegroundServiceCompat(title: String, content: String, useMediaProjection: Boolean = false) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
...
class NotificationActionReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val intentToActivity = Intent("com.example.zap_share.CAST_COMMAND")
...
class ZapShareNativeService : android.app.Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        try {
            ...
            android.util.Log.d("ZapShareService", "onStartCommand entry: title=$title, content=$content, useMediaProjection=$useMediaProjection")
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                val channelId = "zapshare_service_channel"
                val channelName = "ZapShare Background Service"
                ...
                val builder = android.app.Notification.Builder(this, channelId)
                    ...
                    // Use system media icons for compatibility
                    .addAction(android.app.Notification.Action.Builder(android.R.drawable.ic_media_rew, "Rew", pRew).build())
                    .addAction(android.app.Notification.Action.Builder(android.R.drawable.ic_media_play, "Play", pPlay).build())
                    .addAction(android.app.Notification.Action.Builder(android.R.drawable.ic_media_pause, "Pause", pPause).build())
                    .addAction(android.app.Notification.Action.Builder(android.R.drawable.ic_media_ff, "Fwd", pFwd).build())
                ...
                // MediaStyle fallback - CRITICAL: must set actions for compact view
                try {
                    val mediaStyle = android.app.Notification.MediaStyle()
                    // Show Rew, Play, Pause in compact view (indices 0, 1, 2)
                ...
                if (Build.VERSION.SDK_INT >= 29) {
                    try {
                        ...
                        android.util.Log.d("ZapShareService", "Calling startForeground with type: $type")
                        startForeground(1, notification, type)
                    } catch (e: Exception) {
                        android.util.Log.e("ZapShareService", "startForeground(type) failed: ${e.message}")
