...
class NativeVideoPlayerActivity : Activity() {
    ...
    companion object {
        ...
        @JvmStatic
        fun handleCastCommandFromDart(action: String, seekPosition: Double?, volume: Double?, trackIndex: Int? = null): Boolean {
            ...
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(sb: SeekBar, progress: Int, fromUser: Boolean) {
                    if (fromUser && duration > 0) {
                        player?.seekTo(duration * progress / 1000)
                        if (isCastActive) pushCastStatusToFlutter()
                    }
                }
                override fun onStartTrackingTouch(sb: SeekBar) { mainHandler.removeCallbacks(hideControlsRunnable) }
                override fun onStopTrackingTouch(sb: SeekBar) { scheduleHideControls() }
            })
    ...
    private fun initPlayerInternal() {
        ...
        val loadControl = DefaultLoadControl.Builder()
            .setBufferDurationsMs(if (isTvDevice) 60_000 else 50_000, if (isTvDevice) 120_000 else 100_000, if (isTvDevice) 1_500 else 2_500, if (isTvDevice) 3_000 else 5_000)
            .setPrioritizeTimeOverSizeThresholds(true).setBackBuffer(if (isTvDevice) 30_000 else 15_000, true).build()
    ...
    private fun initPlayerSafe() {
        ...
        val loadControl = DefaultLoadControl.Builder().setBufferDurationsMs(50_000, 100_000, 2_500, 5_000).setPrioritizeTimeOverSizeThresholds(true).setBackBuffer(15_000, true).build()
        val renderersFactory = DefaultRenderersFactory(this).apply { setEnableDecoderFallback(true); setExtensionRendererMode(DefaultRenderersFactory.EXTENSION_RENDERER_MODE_ON) }
        player = ExoPlayer.Builder(this).setTrackSelector(trackSelector!!).setLoadControl(loadControl).setRenderersFactory(renderersFactory).build()
        configurePlayer(); attachSurfaceAndLoadMedia()
    }
    ...
    private fun configurePlayer() {
        player?.apply {
            ...
            // Use closest-sync seeking for smoothness (no frame drops on rapid seeks)
            try { setSeekParameters(androidx.media3.exoplayer.SeekParameters.CLOSEST_SYNC) } catch (_: Exception) {}
    ...
    private fun openAudioDeviceMenu() {
        ...
        menuItems = listOf("← Back") + labels.mapIndexed { idx, label ->
    ...
    /** VLC-style seek: accumulates seeks while key is held, shows total, commits on release */
    private fun handleSeekKey(deltaMs: Long) {
        ...
        // Show controls but don't steal focus
        if (!controlsVisible) {
            controlsVisible = true
            updateProgress()
            fadeViews(1f, topBar, topGradient, bottomBar, bottomGradient, centerRow)
        }
        // Reset seek accumulator after key is released (400ms debounce)
