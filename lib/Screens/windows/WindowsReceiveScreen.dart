...
class _WindowsReceiveScreenState extends State<WindowsReceiveScreen> {
  ...
  Future<bool> _isFolderWritable(String folderPath) async {
    try {
      final dir = Directory(folderPath);
      if (!await dir.exists()) {
        await dir.create(recursive: true);
      }
      final testFile = File('$folderPath\\.zapshare_write_test');
      await testFile.writeAsString('test');
      await testFile.delete();
      return true;
    } catch (e) {
      print('Folder $folderPath is not writable: $e');
      return false;
    }
  }

  Future<void> _initDefaultSaveFolder() async {
    final folder = await _getDefaultDownloadFolder();
    final writable = await _isFolderWritable(folder);
    if (writable) {
      _saveFolder = folder;
    } else {
      try {
        final temp = await getTemporaryDirectory();
        _saveFolder = temp.path;
      } catch (_) {
        _saveFolder = 'C:\\Downloads';
      }
    }
    if (mounted) setState(() {});
  }
  ...
  Future<void> _pickCustomFolder() async {
    try {
      ...
      if (selectedDirectory != null) {
        final writable = await _isFolderWritable(selectedDirectory);
        if (!writable) {
          showDialog(
            context: context,
            builder:
    ...
    try {
      ...
      await for (var line in stream) {
        ...
        if (files.isNotEmpty) {
          ...
          // Auto-start download removed per user request
          // _startDownloads();
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          _saveFolder ?? 'Select Download Folder',
                          style: GoogleFonts.outfit(
                            color: Colors.white,
                            fontSize: 14,
                            fontWeight: FontWeight.w600,
                        ...
                        Text(
                          _saveFolder == null
                              ? 'Tap to choose where to save files'
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Ensure ZapShare is allowed through the Windows Defender Firewall to receive/send files on your local network.',
  ...
  Future<void> _fixFirewall() async {
    _showStatus(message: "Configuring Firewall...", icon: Icons.sync);
    try {
      ...
      if (result.exitCode == 0) {
        _showStatus(
          message: "Firewall request sent",
          subtitle: "Please accept UAC elevation prompt if shown.",
          icon: Icons.check_circle_rounded,
          isSuccess: true,
          autoDismiss: const Duration(seconds: 4),
        );
      } else {
        throw Exception("Powershell failed to start process");
      }
    } catch (e) {
      print('Firewall error: $e');
      _showStatus(
        message: "Failed to configure firewall",
        subtitle: "Please allow manually in settings.",
        isError: true,
        autoDismiss: const Duration(seconds: 4),
      );
    }
