...
class _WindowsFileShareScreenState extends State<WindowsFileShareScreen> {
  ...
  Future<void> _checkIpChange() async {
    ...
    if (bestIp != _localIp) {
      debugPrint('🔄 IP address change detected: $_localIp -> $bestIp');
      setState(() {
        _localIp = bestIp;
      });
      if (_files.isNotEmpty) {
        ...
        if (await fsFile.exists()) {
          ...
          try {
            ...
            while (bytesSent < fileSize) {
              ...
              // Moderate flush to manage backpressure without killing speed (every 2MB)
      ...
      (chunk) async {
        ...
        if (buffer[0] >= 32 && buffer[0] < 127) {
          if (buffer.contains(10)) {
            ...
            final consumedCount = lineBytes.length + 1;
            buffer.removeRange(0, consumedCount);
        ...
        if (buffer.length >= 4) {
          ...
          print('📥 TCP Binary: Client requested file index: $index');
