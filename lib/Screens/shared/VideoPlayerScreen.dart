...
class VideoPlayerScreen extends StatefulWidget {
  ...
  /// Optional pre-known duration (useful for HTTP cast streams where MPV probing fails)
...
class _VideoPlayerScreenState extends State<VideoPlayerScreen>
  ...
  // D-pad Settings Menu Navigation
  bool _showSettingsMenu = false;
  int _selectedMenuIndex = 0;
  String? _currentSubMenu; // 'audio', 'subtitle', 'aspect', 'speed'
  ...
  void initState() {
    ...
    // Initialize with pre-known duration if available (fixes HTTP stream duration bugs)
    ...
    _player.errorStream.listen((error) {
      if (mounted && error.isNotEmpty) {
        ...
        // Note: MpvVideoPlayer internally handles codec error fallback chain
        // (mediacodec-copy → mediacodec → no) via its own error listener.
    ...
    // Free CPU for heavy HEVC decoding:
    // - Cast mode: only pause broadcasts (sockets must stay open for remote control)
    // - Local mode: fully stop discovery (close sockets, timers, everything)
    if (widget.castControllerIp != null) {
  ...
  void dispose() {
    ...
    // Restart device discovery when player is closed
  ...
  // ─── Keyboard handler (VLC-style shortcuts + D-pad menu navigation) ──────────────
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    ...
    // Show controls on any D-pad/keyboard input (TV remote support)
    if (!_controlsVisible) {
    ...
    // ═══════════════════════════════════════════════════════════
    // Settings Menu Navigation (D-pad)
    // ═══════════════════════════════════════════════════════════
    ...
    // Menu/Options button: Open settings menu
    ...
    // TV Remote: D-pad Center/Select or Enter: Play/Pause
    if (key == LogicalKeyboardKey.select ||
        key == LogicalKeyboardKey.enter ||
    ...
    // Left arrow: Seek back 10s (Shift: 30s)
    if (key == LogicalKeyboardKey.arrowLeft) {
      final dur = isShift ? _seekDurationLarge : _seekDuration;
      _seekRelative(-dur);
      _showActionIndicator(Icons.fast_rewind_rounded, '-${dur.inSeconds}s');
      return KeyEventResult.handled;
    }

    // Right arrow: Seek forward 10s (Shift: 30s)
    ...
    // Escape or TV Remote Back: Exit fullscreen, or go back
  ...
  List<Map<String, dynamic>> _getSubMenuItems() {
    if (_currentSubMenu == 'audio') {
      return _audioTracks.map((track) {
    ...
    } else if (_currentSubMenu == 'speed') {
      final speeds = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0];
      return speeds.map((speed) {
        return {
          'title': '${speed}x',
          'isActive': (_playbackSpeed - speed).abs() < 0.01,
          'speed': speed,
        };
      }).toList();
    }
  ...
  void _activateMenuItem(int index) {
    ...
    final action = items[index]['action'] as String;
    setState(() {
      _currentSubMenu = action;
      _selectedSubMenuIndex = 0;
    });
  }

  void _activateSubMenuItem(int index) {
    ...
    if (_currentSubMenu == 'audio') {
      ...
      final aspectIndex = _aspectRatios.indexOf(aspectRatio);
      if (aspectIndex >= 0) {
        setState(() {
          _currentAspectRatioIndex = aspectIndex;
          _showSettingsMenu = false;
        });
        final name = aspectRatio['name'] as String;
        final val = aspectRatio['value'] as String;
        ...
        if (_player is NativePlatformMpvPlayer) {
          ...
          if (val == 'fill') {
            ...
            mpv.setProperty('video-aspect-override', '0');
            mpv.setProperty('panscan', '0.0');
          } else if (val != '-1') {
            mpv.setProperty('video-aspect-override', val);
            mpv.setProperty('panscan', '0.0');
        ...
        _showActionIndicator(Icons.aspect_ratio_rounded, 'Aspect', name);
      }
    } else if (_currentSubMenu == 'speed') {
      final speed = item['speed'] as double;
      _setSpeed(speed);
      setState(() => _showSettingsMenu = false);
      _showActionIndicator(Icons.speed_rounded, 'Speed', '${speed}x');
    }
      ...
      child: Scaffold(
        ...
        body: Stack(
          ...
          children: [
              ...
              child: RepaintBoundary(
                child: Builder(
                  builder: (context) {
                    ...
                    Widget videoWidget = _player.buildVideoWidget(fit: fit);
            ...
            // On Linux, pure hardware overlay covers the UI anyway.
            // Delegate all controls to the remote phone.
            if (!Platform.isLinux && _controlsVisible && !_locked) ...[
              _buildTopBar(),
  ...
  Widget _buildSeekBar() {
    double maxMs = _duration.inMilliseconds.toDouble();
    if (_position.inMilliseconds.toDouble() > maxMs) {
      maxMs = _position.inMilliseconds.toDouble();
    }
    if (_isSeeking && _seekPreviewPosition.inMilliseconds > maxMs) {
      maxMs = _seekPreviewPosition.inMilliseconds.toDouble();
    }

    final pos = _position.inMilliseconds.toDouble().clamp(0.0, maxMs);
    final buf = _buffered.inMilliseconds.toDouble().clamp(0.0, maxMs);
