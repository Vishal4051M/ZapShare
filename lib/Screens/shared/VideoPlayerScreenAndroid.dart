...
class _VideoPlayerScreenAndroidState extends State<VideoPlayerScreenAndroid>
  ...
  void initState() {
    ...
    _player.stream.error.listen((error) {
      if (mounted && error.isNotEmpty) {
        ...
        // Detect codec errors → advance hwdec fallback chain
        ...
        if (lowerError.contains('codec') ||
          ...
          _addLog(
            'FALLBACK',
            'Codec error → retrying with hwdec=$nextMode (attempt ${_hwdecAttempt + 1})',
            _LogLevel.warn,
          );
        }
  ...
  Future<void> _openMedia() async {
    ...
    if (_player.platform is native_player.NativePlayer) {
      ...
      if (Platform.isAndroid) {
        ...
        _addLog(
          'CONFIG',
          'hwdec=$hwdec (attempt ${_hwdecAttempt + 1})',
          _LogLevel.info,
        );
  ...
  void _toggleControls() {
    ...
    setState(() {
      ...
      if (_controlsVisible) {
        // Immediately sync display position so seek bar doesn't show stale value
