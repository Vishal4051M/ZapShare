...
class LinuxReceiveScreen extends StatefulWidget {
  final String? autoConnectCode;

  const LinuxReceiveScreen({super.key, this.autoConnectCode});

  @override
  State<LinuxReceiveScreen> createState() => _LinuxReceiveScreenState();
}
...
class _LinuxReceiveScreenState extends State<LinuxReceiveScreen> {
  ...
  Future<String> _getDefaultDownloadFolder() async {
    try {
      ...
      return Platform.isWindows ? 'C:\\Downloads' : '/tmp';
    }
  }
  ...
  Future<void> _fetchFileList(String code) async {
    print('🚀 [Receive] Fetching file list for code: $code');
    if (!_decodeCode(code)) {
      print('❌ [Receive] Code decoding failed');
      return;
    }
    ...
    print('🚀 [Receive] Target IP: $_serverIp, Port: $_serverPort');
  ...
  Future<void> _downloadFile(DownloadTask task) async {
    ...
    try {
      ...
      String savePath = '$_saveFolder${Platform.pathSeparator}$fileName';
      int count = 1;
      while (await File(savePath).exists()) {
        ...
        if (parts.length > 1) {
          ...
          savePath = '$_saveFolder${Platform.pathSeparator}${base}_$count.$ext';
        } else {
          savePath = '$_saveFolder${Platform.pathSeparator}${fileName}_$count';
        }
        count++;
      }
