...
class _AndroidFileListScreenState extends State<AndroidFileListScreen>
  ...
  bool _isDownloadButtonFocused = false;
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    ...
    if (key == LogicalKeyboardKey.arrowUp) {
      ...
      setState(() {
        if (_isDownloadButtonFocused) {
          _isDownloadButtonFocused = false;
          _focusedItemIndex = _fileItems.length - 1;
        } else if (_focusedItemIndex > 0) {
          _focusedItemIndex--;
        }
      });
    ...
    // D-pad Center/Select/Enter: Toggle selection or trigger download
    if (key == LogicalKeyboardKey.select ||
        key == LogicalKeyboardKey.enter ||
        key == LogicalKeyboardKey.space) {
      HapticFeedback.mediumImpact();
      if (_isDownloadButtonFocused && !_downloading) {
        _startDownloads();
      } else if (!_downloading &&
          _focusedItemIndex >= 0 &&
          _focusedItemIndex < _fileItems.length) {
        final file = _fileItems[_focusedItemIndex];
        if (file.status != 'Complete' && file.status != 'Downloading') {
          setState(() => file.isSelected = !file.isSelected);
        } else if (file.status == 'Complete' && file.savePath != null) {
    ...
    if (key == LogicalKeyboardKey.goBack || key == LogicalKeyboardKey.escape) {
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (context) => const AndroidHomeScreen()),
        (route) => false,
      );
      return KeyEventResult.handled;
    }
  ...
  Future<void> cancelProgressNotification(int fileIndex) async {
    await _notificationsPlugin.cancel(id: 2000 + fileIndex);
  }
  ...
  Future<void> _startDownloads() async {
    ...
    try {
      ...
      // Android TV may not support foreground service notifications — ignore
      print('Note: Foreground service start: $e');
    }
    ...
    try {
                ...
                if (!metadataComplete) {
                  ...
                  if (metadataLength != null &&
                    ...
                    if (metadataBytes.length > 4 + metadataLength!) {
                      controller.add(
                        metadataBytes.sublist(4 + metadataLength!),
                      );
                    }
                  }
                } else {
      ...
      try {
        ...
        if (downloadSuccess && widget.useTcp && tcpSocket != null) {
          try {
            print('🛰️ [Receiver] Sending final ACK for $fileName');
            tcpSocket.write('ACK\n');
            await tcpSocket.flush();
            // Give enough time for the bytes to leave the system buffer of the OS
            // Otherwise destroy() might cut them off.
      ...
      if (downloadSuccess) {
        ...
        if (mounted) {
          ...
          // Update foreground service to 100% for this file
          try {
            await FlutterForegroundTask.updateService(
              notificationTitle: "Download Complete",
              notificationText: "${file.name} (100.0%) received.",
            );
          } catch (_) {}

          // Ensure notification is 100%
          await showProgressNotification(
            _fileItems.indexOf(file),
            1.0,
            fileName,
      ...
      // Don't show error if the file was actually fully received
      final actuallyComplete =
          file.savePath != null &&
          file.savePath!.isNotEmpty &&
          File(file.savePath!).existsSync() &&
          file.bytesReceived > 0 &&
          (file.size <= 0 || file.bytesReceived >= (file.size * 0.95).floor());

      if (actuallyComplete) {
        // File was saved successfully — the error was from something else
        // (e.g. FlutterForegroundTask PlatformException on Android TV)
        print('Note: Post-download error ignored (file complete): $e');
        await _addToHistory(
          fileName: file.name,
          fileSize: file.size,
          path: file.savePath!,
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: (widget.useTcp ? Colors.green : Colors.blue).withOpacity(
                  0.4,
                ),
              ),
            ),
          ...
          builder: (context, _) {
            return CustomPaint(
              painter: _PieChartPainter(
                files: _fileItems,
                sectorColors:
                    _fileItems.map((f) => _getFileTypeColor(f.name)).toList(),
  ...
  Widget _buildFileRow(int index) {
    ...
    final isFocused = index == _focusedItemIndex && !_isDownloadButtonFocused;
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
              ...
              child: ClipRRect(
                ...
                child: Stack(
                  children: [
                    Container(
                      decoration: BoxDecoration(
                        color: (isComplete ? Colors.green : color).withOpacity(
                          0.12,
                        ),
                        borderRadius: BorderRadius.circular(10),
                      ),
            ...
            if (isDownloading) ...[
              _miniIconButton(
                file.isPaused ? Icons.play_arrow_rounded : Icons.pause_rounded,
                Colors.white70,
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(
                          ...
                          size: 20,
                          color: _downloading ? Colors.white54 : Colors.black,
                        ),
                        const SizedBox(width: 8),
                        Text(
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < total; i++) {
      ...
      final innerArcPaint =
          Paint()
            ..color =
                sel ? color.withOpacity(0.20) : Colors.white.withOpacity(0.04)
            ..style = PaintingStyle.stroke
            ..strokeWidth = 0.8
            ..strokeCap = StrokeCap.round;
        ...
        final tp = TextPainter(
          text: TextSpan(
            ...
            style: TextStyle(
              ...
              fontSize: total <= 4 ? 13 : (total <= 8 ? 10 : 8),
              fontWeight: FontWeight.w800,
  ...
  void _sector(
    ...
    final path =
        Path()
          ...
          ..lineTo(
            ctr.dx + cos(start + sweep) * iR,
            ctr.dy + sin(start + sweep) * iR,
          )
          ..arcTo(
            Rect.fromCircle(center: ctr, radius: iR),
            start + sweep,
            -sweep,
            false,
          )
