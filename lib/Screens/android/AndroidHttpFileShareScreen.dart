...
class _AndroidHttpFileShareScreenState
  ...
  final List<Socket> _activeTcpClients = []; // Track active TCP clients
  ...
  Future<void> cancelProgressNotification(int fileIndex) async {
    await flutterLocalNotificationsPlugin.cancel(id: 1000 + fileIndex);
  }
  ...
  Future<void> serveSafFile(
    ...
    int chunkSize = 1024 * 1024, // 1MB - optimized for high-bitrate transfers
  }) async {
    ...
    if (rangeHeader != null) {
      ...
      try {
        ...
        if (overallProgress >= 0.999) {
          ...
          print('✅ File fully sent via Range requests: $fileIndex: $fileName');
        }
      }
      return;
    }
      ...
      while (!done && _isSharing) {
        ...
        try {
          ...
          if (chunk == null || chunk.isEmpty) {
            ...
            // Flush periodically to manage backpressure (every 2MB)
  ...
  Future<void> _startSharingSession() async {
    ...
    if (!success) {
      _showModernToast(
        message: 'Failed to start file server. Please try again.',
  ...
  Future<bool> _startListening() async {
    ...
    print('🔄 Server: Attempting to start servers on port $_port...');
    ...
    try {
      ...
      print('⚠️ Warning during server cleanup: $e');
    }
    ...
    try {
      ...
      print('ℹ️ SSL Certificate not available, using plain HTTP');
      sc = null;
    }
    ...
    try {
      ...
      _tcpServer!.listen((client) {
        _handleTcpClient(client).catchError((e) {
          print('❌ Error in TCP client handler: $e');
        });
      });
      print('🚀 TCP Server: Listening on port ${_port + 1}');
    } catch (e) {
      print('❌ Failed to start TCP server: $e');
      // HTTP server started, but TCP failed. We can still function for web,
      // but app-to-app won't work. Let's return false as TCP is critical for the user.
      return false;
    }
  ...
  Future<void> _handleTcpClient(Socket client) async {
    ...
    try {
      ...
      await for (final chunk in client) {
        if (waitingForAck) {
          // Already sent a file, now waiting for ACK on the SAME socket
          try {
            ...
            // Might be more data or some other message, ignore and continue waiting for ACK or closure
          }
          continue;
        }
        ...
        if (buffer.length >= 4) {
          ...
          // Send file then WAIT for ACK in the next loop iteration (same socket)
  ...
  Future<void> _sendFileOverTcp(Socket client, int fileIndex) async {
    ...
    try {
      ...
      // Initialize client download tracking for TCP
      if (!_clientDownloads.containsKey(clientAddress)) {
        _clientDownloads[clientAddress] = {};
        if (!_connectedClients.contains(clientAddress)) {
          _connectedClients.add(clientAddress);
        }
      }

      // Create or update download status for this client
      _clientDownloads[clientAddress]![fileIndex] = DownloadStatus(
      ...
      const chunkSize = 1024 * 1024; // 1MB chunks for high-bitrate throughput
      ...
      if (fileUri.startsWith('content://')) {
        ...
        try {
          ...
          while (!done && _isSharing) {
            ...
            if (chunk == null || chunk.isEmpty) {
              ...
              if (now.difference(lastUpdate).inMilliseconds > 200 ||
                ...
                // Update client-specific download status
                if (_clientDownloads.containsKey(clientAddress) &&
                    _clientDownloads[clientAddress]!.containsKey(fileIndex)) {
                  _clientDownloads[clientAddress]![fileIndex]!.bytesSent =
                      bytesSent;
                  _clientDownloads[clientAddress]![fileIndex]!.progress =
                      progress;
                  _clientDownloads[clientAddress]![fileIndex]!.speedMbps =
                      speedMbps;
                }
          ...
          // Closing stream
          await _channel.invokeMethod('closeStream', {
            'uri': fileUri,
            'streamId': streamId,
          });
        } catch (e) {
      ...
      // Mark as completed for this specific client
      if (_clientDownloads.containsKey(clientAddress) &&
          _clientDownloads[clientAddress]!.containsKey(fileIndex)) {
        final status = _clientDownloads[clientAddress]![fileIndex]!;
      ...
      // ENFORCE 100% UI - Critical fix for the "Stuck at 99%" issue
      _progressList[fileIndex].value = 1.0;
      _bytesSentList[fileIndex] = fileSize;
      if (_completedFiles.length > fileIndex) {
        _completedFiles[fileIndex] = true;
      ...
      Future.delayed(const Duration(seconds: 2), () {
        cancelProgressNotification(fileIndex);
      });

      print('✅ TCP: File sent successfully: $fileName ($bytesSent bytes)');
    } catch (e) {
      print('❌ TCP: Error sending file $fileName: $e');
    }
  ...
  Future<void> _stopServer() async {
    ...
    for (final client in List.from(_activeTcpClients)) {
      try {
  ...
  Future<void> _autoStopSharing() async {
    ...
    print('🤖 Auto-stopping sharing (all transfers complete)');
...
<body>
    ...
    <script>
        ...
        function startDownload(button, fileIndex) {
            ...
            xhr.onprogress = function(event) {
                if (event.lengthComputable) {
                    ...
                    downloadBtn.textContent = `\${Math.round(percentComplete)}%`;
                } else {
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    ...
                    // Create a link and trigger the save dialog
                    ...
                    alert('Download failed. Server may have stopped.');
                    resetBtn();
                }
            };
        ...
        // Auto-refresh every 15 seconds (reduced for stability)
        setInterval(() => {
            if (document.querySelectorAll('.file-item .progress-container[style*="block"]').length === 0) {
                loadFiles();
            }
        }, 15000);
    </script>
              ...
              child: Center(
                child: SizedBox(
                  ...
                  child: SearchPulseWidget(
                    ...
                    size: pulseSize,
                    color: Colors.black, // Consistent dark mode style
                  ),
                ),
              ),
            ),
...
class _ClientProgressSectionState extends State<_ClientProgressSection> {
  ...
  String _getRemainingTime(DownloadStatus status) {
    if (status.speedMbps <= 0 || status.progress <= 0 || status.progress >= 1.0)
      return "";

    final remainingBytes = status.fileSize * (1.0 - status.progress);
    final remainingMB = remainingBytes / (1024 * 1024);
    final remainingSeconds = (remainingMB / status.speedMbps).round();
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      if (status.speedMbps > 0) ...[
                        SizedBox(width: 6),
                        Text(
                          '${status.speedMbps.toStringAsFixed(1)} Mbps${status.progress < 1.0 ? " • ${_getRemainingTime(status)}" : ""}',
                          style: GoogleFonts.outfit(
                            color: Colors.grey[500],
                            fontSize: 12,
                            fontWeight: FontWeight.w500,
                  ...
                  TweenAnimationBuilder<double>(
                    tween: Tween<double>(end: status.progress),
