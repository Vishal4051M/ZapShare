...
class UdpAudioReceiverService {
  ...
  Future<void> start() async {
    ...
    try {
            ...
            if (datagram.data.length > 10) {
              ...
              while (_jitterList.length > targetCushion) {
                ...
                if (_lastReleasedSeq != null) {
                  ...
                  if (diff < 0) {
                    print('⚠️ Discarding late packet: seq=${packet.seq}, expected=${(_lastReleasedSeq! + 1) % 65536}');
                    continue;
                  }
                  ...
                  if (packet.seq != expected) {
                    ...
                    if (missing > 0 && missing < 15) { 
                       // PLC: Simple silence insertion for missing frames
                       for (int i = 0; i < missing; i++) {
                         _audioDataController.add(Uint8List(packet.data.length));
                       }
                    }
                  }
                }
      ...
      _httpServer = await HttpServer.bind(InternetAddress.loopbackIPv4, HTTP_PORT);
  ...
  Uint8List _createWavHeader(int dataSize) {
    ...
    header.setUint32(28, SAMPLE_RATE * CHANNELS * BITS_PER_SAMPLE ~/ 8, Endian.little); // ByteRate
    header.setUint16(32, CHANNELS * BITS_PER_SAMPLE ~/ 8, Endian.little); // BlockAlign
