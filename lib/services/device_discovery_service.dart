...
class CastControl {
  final String
  action; // play, pause, seek, volume, stop, setAudioTrack, setSubtitleTrack
...
class DeviceDiscoveryService {
  ...
  Timer?
  _multicastRenewTimer; // Periodic multicast lock renewal for long-running cast sessions
  ServerSocket?
  _tcpControlServer; // TCP control channel for reliable cast commands
  bool _isRunning = false;
  bool _isRestarting = false; // Flag to prevent multiple restart attempts
  bool _isStopping =
      false; // Flag to prevent restart when stop() is intentional
  ...
  final StreamController<bool> _audioActiveController =
  ...
  Future<void> startLanAudioSender(List<String> targetIps) async {
    if (!Platform.isAndroid) return;
    try {
      final status = await _nativeChannel.invokeMethod('startLanAudioSender', {
        'ips': targetIps,
      });
      if (status == true) {
  ...
  Future<void> startLanAudioReceiver({
    int port = 50005,
    int cushionMs = 40,
  }) async {
    if (!Platform.isAndroid) return;
    ...
    if (sdp.startsWith('LAN_AUDIO_STREAM_PORT:')) {
      ...
      final cushion =
          int.tryParse(
            parts
                .firstWhere(
                  (p) => p.startsWith('cushion='),
                  orElse: () => 'cushion=$_preferredCushionMs',
                )
  ...
  Future<void> sendNativeSystemAudioOffer(
    String targetIp,
    int port, {
    int cushionMs = 20,
  }) async {
    ...
    final payload = {
      ...
      'deviceName': _myDeviceName,
      'sdp':
          'LAN_AUDIO_STREAM_PORT:$port;cushion=$cushionMs', // Special flag for new LAN audio
    };
  ...
  Future<void> sendHttpAudioOffer(String targetIp, String audioUrl) async {
    ...
    final payload = {
      ...
      'deviceId': _myDeviceId,
      'deviceName': _myDeviceName,
      'sdp': 'HTTP_AUDIO_STREAM_URL:$audioUrl',
    };
    final msgData = utf8.encode(jsonEncode(payload));
  ...
  Future<void> _sendAudioOfferPayload(
    String targetIp,
  ...
  Future<void> _sendAudioAnswerPayload(
    String targetIp,
    _AudioSession session,
  ) async {
    final desc = await session.pc.getLocalDescription();
