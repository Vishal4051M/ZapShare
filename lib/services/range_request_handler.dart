...
class RangeRequestHandler {
  ...
  static Future<void> _servePartialFile(
    ...
    print(
      '📦 Serving range: bytes $start-$end/$fileSize (${contentLength} bytes)',
    );

    await _streamFileContent(
      response,
      uri,
      start,
      end,
    ...
    try {
      ...
      if (start > 0) {
        try {
          ...
          print(
            '⚠️ Seek failed: $e, reading and discarding $start bytes instead',
          );
          // Fallback: read and discard bytes until start position
          int toSkip = start;
          while (toSkip > 0) {
            final skipSize = toSkip > CHUNK_SIZE ? CHUNK_SIZE : toSkip;
            final discardChunk = await channel.invokeMethod<Uint8List>(
              'readChunk',
              {'uri': uri, 'streamId': streamId, 'size': skipSize},
            );
      ...
      print(
        '📦 Starting stream: bytes $start-$end (${bytesToRead} bytes total)',
      );
      ...
      while (bytesToRead > 0) {
        ...
        if (chunk == null || chunk.isEmpty) {
          print(
            '⚠️ End of stream at position $currentPosition (expected $end)',
          );
          break;
        }
        ...
        final bytesToSend =
            chunk.length > bytesToRead ? bytesToRead : chunk.length;
        ...
        if (totalSent % (1024 * 1024) < CHUNK_SIZE) {
          final progress = (totalSent / (end - start + 1) * 100)
              .toStringAsFixed(1);
          print(
            '  📤 Range $start-$end: ${totalSent} / ${end - start + 1} bytes ($progress%)',
          );
        }
      }
