...
class _CastRemoteControlWidgetState extends State<CastRemoteControlWidget>
  ...
  String? _activeAudioTrackLabel; // track by name (e.g. 'English') for better matching
  ...
  DateTime? _lastManualActionTime; // debounce: skip auto-correction shortly after user seeks
  int? _pendingLocalAudioTrack; // remember requested track before local audio player is ready
  ...
  void initState() {
      ...
      if (ipMatch || acceptAnyWhileConnecting) {
        if (!status.active) {
          if (_connected) {
            ...
            _stopLocalAudio();
            debugPrint('[CastRemote] TV signaled cast end, stopping local audio');
          }
          return;
        }
        ...
        _handleAudioOutputChange(status.audioOutput, status.position, status.isPlaying);
        ...
        if (_isCastSourceAudio &&
          ...
          if (_localAudioReady) {
            _switchLocalAudioTrack(status.activeAudioTrack ?? 0, label: status.activeAudioTrackLabel);
          } else {
    ...
    _positionAnimTimer =
      ...
      setState(() {
        ...
        _animatedPosition =
            (_animatedPosition + 0.1).clamp(0.0, safeDuration > 0 ? safeDuration : 1.0);
      });
    });
    ...
    _connectionTimer = Timer.periodic(const Duration(seconds: 3), (_) {
      ...
      if (!_connected || (_lastStatusTime != null && now.difference(_lastStatusTime!) > const Duration(seconds: 10))) {
        _sendControl('ping');
      }
      ...
      if (_connected && _lastStatusTime != null && now.difference(_lastStatusTime!) > const Duration(seconds: 30)) {
        if (mounted) {
          setState(() => _connected = false);
          _stopLocalAudio();
          debugPrint('[CastRemote] Lost connection to TV (30s timeout), stopping local audio');
        }
      }
    });

    // Handle Remote Commands from Notification (play/pause/forward/rewind)
    const MethodChannel('zapshare.saf').setMethodCallHandler((call) async {
      if (call.method == 'remoteCommand') {
        ...
        switch (action) {
          ...
          case 'zapshare.ACTION_REWIND':
            _sendControl('seek', seekPosition: (_position - 30).clamp(0, _duration));
            break;
        }
        return null;
      }
  ...
  void _sendControl(String action,
      {double? seekPosition, double? volume, int? trackIndex}) {
  ...
  void _showCastInfo(String msg) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(children: [
          const Icon(Icons.speaker_phone_rounded, color: Colors.black, size: 18),
          const SizedBox(width: 8),
          Expanded(child: Text(msg, style: const TextStyle(color: Colors.black, fontSize: 12))),
        ]),
  ...
  void _handleAudioOutputChange(String? audioOutput, double position, bool isPlaying) {
    ...
    if (wantCastSource && !_isCastSourceAudio) {
      ...
      _lastManualActionTime = DateTime.now(); // suppress sync interference during startup
  ...
  Future<void> _startLocalAudio(double position, bool isPlaying) async {
    ...
    if (player.platform is NativePlayer) {
      ...
      try {
        ...
        await np.setProperty('demuxer-max-bytes', '256MiB'); // 256MB demuxer buffer
        await np.setProperty('demuxer-max-back-bytes', '64MiB');
        await np.setProperty('demuxer-readahead-secs', '300.0'); // 5 minutes readahead
        await np.setProperty('audio-buffer', '3.0'); // 3 second audio-specific buffer
    ...
    tracksSub = player.stream.tracks.listen((tracks) {
      ...
      final realAudio = tracks.audio.where((t) => t.id.isNotEmpty && t.id.toLowerCase() != 'auto' && t.id != 'no').toList();
    ...
    final elapsedSecs = DateTime.now().difference(_localAudioStartedAt!).inMilliseconds / 1000.0;
    final targetPos = (_position + (_isPlaying ? elapsedSecs : 0))
        .clamp(0.0, _duration > 0 ? _duration : double.infinity);
    ...
    var tracks = player.state.tracks.audio.where((t) => t.id.isNotEmpty && t.id.toLowerCase() != 'auto' && t.id != 'no').toList();
    if (tracks.isEmpty) {
      await Future.delayed(const Duration(milliseconds: 400));
      tracks = player.state.tracks.audio.where((t) => t.id.isNotEmpty && t.id.toLowerCase() != 'auto' && t.id != 'no').toList();
    }
    ...
    if (tracks.isNotEmpty) {
      ...
      if (targetLabel != null) {
        selectedTrack = tracks.firstWhere(
          (t) => (t.title?.toLowerCase() == targetLabel.toLowerCase()) || 
      ...
      final trackDesc = selectedTrack.title ?? selectedTrack.language ?? 'Track ${actualIdx + 1}';
      debugPrint('[CastSource] Selected: $trackDesc (ID: ${selectedTrack.id}, idx: $actualIdx)');
    ...
    debugPrint('[CastSource] playback started at ${targetPos.toStringAsFixed(1)}s');
    } finally {
  ...
  void _syncLocalAudio() {
    ...
    final recentManualAction = lastAction != null &&
        DateTime.now().difference(lastAction).inMilliseconds < 1000;

    // Extrapolate TV's current position from last known status
    final timeSinceStatus = _lastStatusTime != null
        ? DateTime.now().difference(_lastStatusTime!).inMilliseconds / 1000.0
        : 0.0;
    final expectedPos = _isPlaying
        ? (_position + timeSinceStatus).clamp(0.0, _duration > 0 ? _duration : double.infinity)
        : _position;
    ...
    final diff = localPos - expectedPos; // positive = local is AHEAD, negative = local is BEHIND
    ...
    if (absDiff > 2.0 && !player.state.buffering) {
      debugPrint('[CastSource] Hard sync (4-dec): local=${localPos.toStringAsFixed(4)}s, expected=${expectedPos.toStringAsFixed(4)}s, diff=${diff.toStringAsFixed(4)}s');
      ...
      Future.delayed(const Duration(milliseconds: 1200), () {
        if (mounted && _isCastSourceAudio) {
          setState(() { _localAudioReady = true; });
          if (wasPlayingGlobal) {
            _sendControl('play');
            player.play();
          }
      ...
      double rate = 1.0;
      if (diff > 0.6) rate = 0.94;
      else if (diff > 0.1) rate = 0.97;
      else if (diff > 0.02) rate = 0.99;
      else if (diff < -0.6) rate = 1.06;
      else if (diff < -0.1) rate = 1.03;
      else if (diff < -0.02) rate = 1.01;
      
      if (player.state.rate != rate) {
        debugPrint('[CastSource] Fine-sync rate: $rate (diff: ${diff.toStringAsFixed(4)}s)');
        player.setRate(rate);
      }
    } else {
  ...
  Future<void> _switchLocalAudioTrack(int trackIndex, {String? label}) async {
    ...
    try {
      ...
      final realTracks = player.state.tracks.audio.where((t) => t.id.isNotEmpty && t.id != 'auto').toList();
      ...
      if (player.platform is NativePlayer) {
        ...
        if (label != null) {
          track = realTracks.firstWhere(
            ...
            orElse: () => realTracks[trackIndex.clamp(0, realTracks.length - 1)],
          );
        } else {
        ...
        debugPrint('[CastSource] Switching: aid=${localIdx + 1} label=$label matched=${track.title}');
        ...
        if (mounted) {
          _logCastStep('Ready');
          _showCastInfo('Switched to ${track.title ?? 'Track ${localIdx + 1}'}');
        }
      } else {
      ...
      debugPrint('[CastSource] Optimized switch failed, falling back to full restart: $e');
  ...
  String _formatDuration(double seconds) {
    ...
    final ms = (seconds * 1000).clamp(0.0, 3600000000.0).toInt(); // Limit to 1000 hours
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        ...
                        style: GoogleFonts.outfit(
                          color: _connected ? Colors.white38 : (_lastStatusTime == null ? Colors.white30 : Colors.orangeAccent.withValues(alpha: 0.7)),
                          fontSize: 11,
                        ),
                      ),
                      if (_isCastSourceAudio) ...[
                        const SizedBox(width: 6),
                        Icon(Icons.speaker_phone_rounded, color: _accentColor, size: 12),
                        const SizedBox(width: 2),
                        Text(
                          _localAudioReady ? 'Phone Audio' : 'Starting...',
                          style: GoogleFonts.outfit(
                            color: _localAudioReady
            ...
            IconButton(
              onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => CastRemoteControlScreen(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildControlBtn(
            ...
            size: 22,
            onTap: () {
              final newPos = (_position - 30).clamp(0.0, _duration > 0 ? _duration : 0.0);
          ...
          _buildControlBtn(
            ...
            size: 26,
            onTap: () {
              final newPos = (_position - 10).clamp(0.0, _duration > 0 ? _duration : 0.0);
              _sendControl('seek', seekPosition: newPos);
              _syncLocalAudioNow(seekTo: newPos);
              setState(() {
                _position = newPos;
          ...
          GestureDetector(
            onTap: () {
              final action = _isPlaying ? 'pause' : 'play';
              _sendControl(action);
              _syncLocalAudioNow(playing: !_isPlaying);
            },
            child: Container(
              width: 48,
              height: 48,
              ...
              if (_subtitleTracks.isNotEmpty)
                Expanded(
                  child: _buildTrackButton(
                    ...
                    label: _activeSubtitleTrack != null &&
  ...
  Widget _buildTrackPicker(
      List<String> tracks, int? activeIdx, void Function(int) onSelect) {
    return Container(
