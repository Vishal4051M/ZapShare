...
DWORD MpvWindow::LaunchMpv(const std::wstring& mpv_executable_path, const std::string& ipc_pipe_name) {
  ...
  // Network streaming cache — low latency by default (overridden by Dart for files)
  command += L" --audio-buffer=0.04";
  command += L" --cache=no";
  command += L" --cache-pause=no";
  command += L" --demuxer-max-bytes=1048576";
  command += L" --demuxer-readahead-secs=0";
  command += L" --vd-lavc-threads=1";
  command += L" --demuxer-lavf-analyzeduration=0.01";
  command += L" --demuxer-lavf-probesize=1024";
  command += L" --video-latency-hacks=yes";
  command += L" --stream-buffer-size=32k";
